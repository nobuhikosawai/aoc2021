...
const bingoBoards = inputs.splice(1).reduce((acc, input) => {
...
const isBingo = (board) => {
  ...
  if (!rowBingo) {
    return [...Array(board[0].length)].some((_, i) => board.every(row => row[i] === 'x'));
  }
  return true;
}
...
for (num of selectedNumbers) {
  ...
  const bingoBoard = bingoBoards.find(board => isBingo(board));
